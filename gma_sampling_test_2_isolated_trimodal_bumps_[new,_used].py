# -*- coding: utf-8 -*-
"""GMA sampling test 2: isolated Trimodal bumps [new, used].ipynb
...
eta = 0.01 # 0.5  # Initial learning rate (eta_0) for the diminishing schedule
...
def log_unnormalized_p_jax(z):
    ...
    term2 = 0.3 * jax_norm.pdf(z, loc=5.0, scale=0.2)
    term3 = 0.2 * jax_norm.pdf(z, loc=-5.0, scale=0.2)
...
def unnormalized_p_torch(theta):
    ...
    norm1 = torch.exp(-0.5 * ((theta - 5.0) / 0.2)**2) / (0.2 * (2 * torch.pi)**0.5)
    norm2 = torch.exp(-0.5 * ((theta + 5.0) / 0.2)**2) / (0.2 * (2 * torch.pi)**0.5)
...
def log_unnormalized_p_pytensor(theta):
    ...
    term2 = 0.3 * pm.math.exp(pm.logp(pm.Normal.dist(mu=5.0, sigma=0.2), theta))
    term3 = 0.2 * pm.math.exp(pm.logp(pm.Normal.dist(mu=-5.0, sigma=0.2), theta))
