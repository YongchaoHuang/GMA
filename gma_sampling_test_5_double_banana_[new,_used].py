# -*- coding: utf-8 -*-
"""GMA sampling test 5: Double Banana [new, used].ipynb
...
# --- GMA Hyperparameters ---
N_sqrt = 25 # Create a 25x25 grid of components
N = N_sqrt**2  # Total number of Gaussian components (625)
M = 20  # Samples per Gaussian
K = 600  # Number of iterations for weight updates
...
# --- DOUBLE BANANA DENSITY: Define the 2D double banana target density ---
...
def unnormalized_p(z):
    ...
    x1 = z[..., 0]
    x2 = z[..., 1]
    log_term = np.log(np.exp(-2 * (x1 - 2) ** 2) + np.exp(-2 * (x1 + 2) ** 2))
    return np.exp(-2 * ((x1 ** 2 + x2 ** 2) - 3) ** 2 + log_term)
...
def log_unnormalized_p_jax(z):
    ...
    log_term = jax.scipy.special.logsumexp(jnp.array([-2 * (x1 - 2) ** 2, -2 * (x1 + 2) ** 2]))
    return -2 * ((x1 ** 2 + x2 ** 2) - 3) ** 2 + log_term

def unnormalized_p_torch(z):
    # PyTorch version for LMC
    x1 = z[0]
    x2 = z[1]
    log_term = torch.logsumexp(torch.stack([-2 * (x1 - 2) ** 2, -2 * (x1 + 2) ** 2]), dim=0)
    return torch.exp(-2 * ((x1 ** 2 + x2 ** 2) - 3) ** 2 + log_term)

def log_unnormalized_p_pytensor(z):
    # PyTensor version for ADVI
    x1 = z[0]
    x2 = z[1]
    # Use pm.math.logsumexp for numerical stability
    log_term = pm.math.logsumexp(pm.math.stack([-2 * (x1 - 2) ** 2, -2 * (x1 + 2) ** 2]))
    return -2 * ((x1 ** 2 + x2 ** 2) - 3) ** 2 + log_term
...
lmc_samples = langevin_monte_carlo(z0=np.zeros(d), total_steps=total_samples, lr=0.005)
