# -*- coding: utf-8 -*-
"""GMA sampling test 3: 2D4modal Gaussian [new, used].ipynb
...
# --- GMA Hyperparameters ---
N_sqrt = 5 # Create a 5x5 grid of components
N = N_sqrt**2  # Total number of Gaussian components (25)
M = 100  # Samples per Gaussian
K = 120  # Number of iterations for weight updates
eta = 0.05  # Initial learning rate for 2D case
z_range = (-6, 6) # Support range for initializing component means
cov_scale = 0.1 # 1.0, 0.1
...
log_p_target = np.log(unnormalized_p(flat_samples) + 1e-12)
...
# ##################################################################
# 2D CHANGE: Define parameters for the 2D, 4-modal Gaussian mixture
d = 2 # Dimension
means_target = [np.array([-3, 3]), np.array([3, 3]), np.array([-3, -3]), np.array([3, -3])]
covs_target = [np.array([[1.0, 0.8], [0.8, 1.0]]),
               np.array([[1.0, -0.8], [-0.8, 1.0]]),
               np.array([[1.0, 0.0], [0.0, 0.2]]),
               np.array([[0.2, 0.0], [0.0, 1.0]])]
weights_target = [0.3, 0.3, 0.2, 0.2]

# --- Target distribution functions for different libraries ---

def unnormalized_p(z):
    # 2D CHANGE: NumPy version for MH, GMA
    pdf_val = 0
    for w, m, c in zip(weights_target, means_target, covs_target):
        pdf_val += w * multivariate_normal.pdf(z, mean=m, cov=c)
    return pdf_val

def log_unnormalized_p_jax(z):
    # 2D CHANGE: JAX version for HMC, SVGD, GM-ADVI
    log_probs = []
    for w, m, c in zip(weights_target, means_target, covs_target):
        log_probs.append(jnp.log(w) + jax_multivariate_normal.logpdf(z, mean=m, cov=c))
    return jax.scipy.special.logsumexp(jnp.array(log_probs))

def unnormalized_p_torch(z):
    # 2D CHANGE: PyTorch version for LMC
    pdf_val = 0
    for w, m, c in zip(weights_target, means_target, covs_target):
        m_torch = torch.tensor(m, dtype=torch.float32)
        c_torch = torch.tensor(c, dtype=torch.float32)
        dist = torch.distributions.MultivariateNormal(m_torch, c_torch)
        pdf_val += w * torch.exp(dist.log_prob(z))
    return pdf_val

def log_unnormalized_p_pytensor(z):
    # 2D CHANGE: PyTensor version for ADVI
    log_probs = []
    for w, m, c in zip(weights_target, means_target, covs_target):
        log_probs.append(pm.math.log(w) + pm.logp(pm.MvNormal.dist(mu=m, cov=c), z))
    return pm.math.logsumexp(pm.math.stack(log_probs))
...
# --- GMA Hyperparameters ---
N_sqrt = 15 # Create a 5x5 grid of components
N = N_sqrt**2  # Total number of Gaussian components (25)
M = 30  # Samples per Gaussian
K = 300  # Number of iterations for weight updates
eta = 0.1  # Initial learning rate for 2D case
z_range = (-6, 6) # Support range for initializing component means
cov_scale = 0.1
...
mh_samples = mh_sampler(n_iter=total_samples, initial_point=np.zeros(d), proposal_cov=np.eye(d))
...
lmc_samples = langevin_monte_carlo(z0=np.zeros(d), total_steps=total_samples, lr=0.1, noise_scale=0.02)
...
# 2D CHANGE: Use 2D initial particles
...
with advi_model:
    ...
    mean_field_approx = pm.fit(n=5000, method='advi', random_seed=fit_seed)
...
ax1.scatter(flat_samples[:, 0], flat_samples[:, 1], alpha=0.1, label='Initial samples', color='gray', s=5)
ax1.scatter(gma_samples[:, 0], gma_samples[:, 1], alpha=0.3, label='Ensemble samples', color='green', s=5)
ax1.set_title(f'GMA Results (N={N}, K={K}, M={M}, Cov={cov_scale}, $\eta_0$={eta}, time={gma_time})')
ax1.set_xlabel('$z_1$')
ax1.set_ylabel('$z_2$')
ax1.legend()
ax1.set_xlim(z_range)
