# -*- coding: utf-8 -*-
"""GMA sampling test 7: Neal's funnel [new, used].ipynb
...
z1_range = (-5, 5) # Asymmetric range for x-axis
z2_range = (-8, 6) # Asymmetric range for y-axis
cov_scale = 0.03 # Use a very small covariance for high resolution

# --- Initialize fixed means and covariances ---
# 2D CHANGE: Create an asymmetric grid for the funnel shape
grid_points_z1 = np.linspace(z1_range[0], z1_range[1], N_sqrt)
grid_points_z2 = np.linspace(z2_range[0], z2_range[1], N_sqrt)
xx, yy = np.meshgrid(grid_points_z1, grid_points_z2)
...
x = np.linspace(z1_range[0], z1_range[1], 200)
y = np.linspace(z2_range[0], z2_range[1], 200)
X, Y = np.meshgrid(x, y)
pos = np.dstack((X, Y))
...
ax1.set_xlim(z1_range)
ax1.set_ylim(z2_range)
...
# --- NEAL'S FUNNEL DENSITY: Define the 2D Neal's Funnel target density ---
...
def log_unnormalized_p_jax(z):
    ...
    x = z[0]
    y = z[1]
    log_p_y = jax_norm.logpdf(y, 0, sigma_funnel)
    log_p_x_given_y = jax_norm.logpdf(x, 0, jnp.exp(y / 2))
    return log_p_x_given_y + log_p_y

def unnormalized_p_torch(z):
    # PyTorch version for LMC
    x = z[0]
    y = z[1]
    p_y = torch.exp(torch.distributions.Normal(0, sigma_funnel).log_prob(y))
    p_x_given_y = torch.exp(torch.distributions.Normal(0, torch.exp(y / 2)).log_prob(x))
    return p_x_given_y * p_y
...
def log_unnormalized_p_pytensor(z):
    ...
    x = z[0]
    y = z[1]
    log_p_y = pm.logp(pm.Normal.dist(0, sigma_funnel), y)
    log_p_x_given_y = pm.logp(pm.Normal.dist(0, pm.math.exp(y / 2)), x)
    return log_p_x_given_y + log_p_y
